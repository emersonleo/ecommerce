	...
	<div class="row">
		...
		<div class="card col-md-12" style="margin-top: 10px; margin-bottom: 10px">
			<div class="card-body">
				...
				<button class="btn btn-success form-group">
						Comprar
				</button>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function construirTabelaCarrinho(Produtos){
		...
		for (var i = 0; i <= Produtos.length - 1; i++) {
			...
			tbodyNovo += "<tr>" + tdProduto + "<td class='col-md-3'> <input id = "+ i + " min='0' data-preco= " + parseFloat(Produtos[i].preço).toFixed(2) + " style='width:50%'type='number' onchange='mudarValor(this)' value='" + Produtos[i].quantidade_carrinho + "'> </td> <td class='col-md-3'> <h6" +  Produtos[i].id + "valor" + "> R$ " + (parseFloat(Produtos[i].preço)*parseInt(Produtos[i].quantidade_carrinho)).toFixed(2) + " </h6></td> </tr>"
